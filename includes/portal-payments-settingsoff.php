...
function casanova_payments_render_settings_page(): void {
  ...
  if ($tab === 'payments') {
    ...
    echo '<tr><td><code>[casanova_proximo_viaje]</code></td><td>Próximo viaje del cliente (título, fechas, días restantes y resumen de importes).</td><td><code>[casanova_proximo_viaje variant="hero"]</code> (dashboard), <code>variant="compact"</code></td></tr>';
    echo '<tr><td><code>[casanova_card_pagos]</code></td><td>Card “Estado de pagos” del expediente activo o próximo viaje.</td><td><code>source="auto|current|next"</code>, <code>cta="both|pagar|detalle|none"</code></td></tr>';
    echo '<tr><td><code>[casanova_card_proxima_accion]</code></td><td>Card “Próxima acción” (prioriza depósito si aplica, si no pago pendiente, si no “todo listo”). Incluye el viaje al que corresponde.</td><td><code>source="auto|current|next"</code>, <code>tab_pagos="pagos"</code>, <code>tab_facturas="facturas"</code></td></tr>';
